...
| Modal not clickable from ARIA tree | Modal is a DOM portal outside the accessibility tree | Use `bin/click-css` with CSS selector |
| Framework event handler not firing | ARIA-ref click doesn't dispatch framework event | Use `bin/click-css` with the framework's selector (e.g. `[data-action=...]`) |
